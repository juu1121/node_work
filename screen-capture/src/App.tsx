...
declare global{
  interface Window{
    api:{
      ...
      onCapturedData : (a:(imageData:string)=>void)=>void //매개변수에 이미지 데이터를 전달받는 콜백함수 전달
    }
  }
}
...
function App() {
  ...
  useEffect(()=>{
    ...
    // main process 에서 "captured-data"이벤트가 발행했을떄 실행할 함수 전달
  ...
  const handleCapture = async ()=>{
    //result 는 capture 된 이미지의 data url 문자열이다. 
...
const handleMouseDown = (e: React.MouseEvent) => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (ctx) {
      // 사각형을 그린다.
      //  .clearRect(x, y, width, height)  사각형 영역에 그려진 내용을 모두 지우기 (캔버스영역지우기기)
